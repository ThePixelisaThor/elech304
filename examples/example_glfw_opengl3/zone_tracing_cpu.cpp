...
void generate_mirrors(vec2 fake_rx, int& current_index, Wall* walls, int& wall_count, int current_reflection_count,
    ...
    /* fake_rx : current mirror in the tree of rx
    ...
    if (current_reflection_count == reflection_count) {
        for (int w = 0; w < wall_count; w++) {
            ...
            new_fake_rx = getMirrorWithWall_cpu(fake_rx, walls[w].fancy_vector);
            all_mirrors[current_index] = new_fake_rx;
        ...
        for (int w = 0; w < wall_count; w++) {
            ...
            generate_mirrors(new_fake_rx, current_index, walls, wall_count, current_reflection_count,
...
void generate_all_mirrors(int& reflection_count, vec2* all_mirrors, int* each_reflection_count, int* walls_to_reflect,
    Wall* walls, int& wall_count, int& mirror_count, vec2& rx, int& total_count) {
    /* reflection_count : max reflection allowed
     * all_mirrors : array containing all the positions of fake_rx (+ rx) to aim for
     * each_reflection_count : array containing for each direction the number of reflections it should do (0 for rx, 1 for rx', 2 for rx'', etc)
     * walls_to_reflect : array of int arrays of max size reflection_count, containing for each direction the walls it should reflect
    ...
    generate_mirrors(rx, mirror_count, walls, wall_count, 0, reflection_count, 0, all_mirrors, each_reflection_count,
        walls_to_reflect, -1);
}
...
void compute_ray_cpu(vec2 &tx, vec2 &rx, vec2 direction, Ray &ray, bool &hit, Wall *walls, int wall_count,
     ...
     * walls : array containing all walls
     * wall_count : number of walls
     * walls_to_reflect : contains the sequence of wall to hit (inverted because we mirrored tx)
     * max_reflexion : max reflexion allowed (0 for rx, 1 for rx', etc)
     * 
    ...
    vec2 current_origin(tx); // starts from tx obviously
    ...
    vec2 current_direction_to_rx = normalize(rx - current_origin); // changes after each reflexion
    ...
    while (true) { // no recursion !!!
        // is it hitting RX, accounting for rounding errors
        if (current_direction_normalized.x < current_direction_to_rx.x + 0.000001 && current_direction_normalized.x > current_direction_to_rx.x - 0.000001 &&
            current_direction_normalized.y < current_direction_to_rx.y + 0.000001 && current_direction_normalized.y > current_direction_to_rx.y - 0.000001 &&
            reflexion_left == 0) {
            distance_to_rx = length(rx - current_origin);

            // a wall could be between the ray reflex and RX
            found = true;
            hit_point = getIntersection_cpu(current_direction, current_origin, walls, wall_count, best_wall, found, distance_to_rx);
            ...
            if (found) {
                ...
                // the ray hits rx before anything else
                ray = Ray(current_origin, rx, energy, total_distance + (distance_to_rx / 10.f));
                hit = true;
                break;
            }
        }

        // if the direction doesn't correspond to hitting rx
        found = true;
        ...
        if (found) {
            ...
            if (reflexion_left != 0 && walls_to_reflect[reflexion_left - 1] == best_wall.id) {
                ...
                reflexion_left--;
                current_direction_to_rx = normalize(rx - current_origin);
...
void compute_all_rays_hitting_rx_cpu(int reflection_count, Wall *walls, int wall_count, vec2 tx, vec2 rx, Ray *rays_hitting_rx, int &ray_count) {
    /* reflection_count : max reflection allowed
     * walls : array of all the walls
     * wall_count : number of walls
     *
    ...
    vec2* all_rx_mirrors = new vec2[max_count]; // il y a moyen de calculer tous les tx mirrors, faire le chemin à l'envers mais c'est chiant je pense
    ...
    generate_all_mirrors(reflection_count, all_rx_mirrors, each_reflection_count, walls_to_reflect,
        walls, wall_count, mirror_count, rx, max_count);

    bool hit;
    ray_count = 0;

    for (int m = 0; m < mirror_count; m++) {
        compute_ray_cpu(tx, rx, all_rx_mirrors[m] - tx, rays_hitting_rx[ray_count], hit, walls, wall_count, &walls_to_reflect[m * reflection_count],
