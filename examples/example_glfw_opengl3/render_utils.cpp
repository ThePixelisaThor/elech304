...
void draw_arrays_rect(GLuint VBOs[], GLuint CBOs[], int length) {
    for (int i = 0; i < length; i++) draw_array_rect(VBOs[i], CBOs[i]);
}

void draw_array_rect(unsigned int VBO, unsigned int CBO) {
    glEnableVertexAttribArray(0);
    glBindBuffer(GL_ARRAY_BUFFER, VBO);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)0); // le vecteur d'attribut contient toujours 3 infos !
